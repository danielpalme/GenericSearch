...
{
    [TestClass]
    public class SearchExtensions_AntlrTest
    {
        ...
        public void GetMatches_NullAsInput_ThrowsArgumentNullException()
        {
            ...
            var matches = sampleInput.FilterUsingAntlr(d => d.Title, "Test").ToArray();
        }

        [TestMethod]
        public void GetMatches_NullAsSearchTerm_AllElementsReturned()
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, null).ToArray();
        ...
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, string.Empty).ToArray();
            Assert.AreEqual(this.sampleInput.Count(), matches.Length);
        }
        ...
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "(Test and Test").ToArray();
        }

        [TestMethod]
        public void GetMatches_SingleWordAsSearchTerm_CorrectElementsReturned()
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "ten").ToArray();
        ...
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "seven ten").ToArray();
            Assert.AreEqual(1, matches.Length);

            matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "seven and ten").ToArray();
            Assert.AreEqual(1, matches.Length);

            matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "seven & ten").ToArray();
            Assert.AreEqual(1, matches.Length);
        }
        ...
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "seven or ten").ToArray();
        ...
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "not ten").ToArray();
            Assert.AreEqual(9, matches.Length);

            matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "- ten").ToArray();
            Assert.AreEqual(9, matches.Length);
        ...
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "\"NiNe teN\"").ToArray();
            Assert.AreEqual(1, matches.Length);
        }
        ...
        {
            var matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "(two or ten) and six").ToArray();
            Assert.AreEqual(5, matches.Length);

            matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "two or ten and six").ToArray();
            Assert.AreEqual(9, matches.Length);

            matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "not (two or ten)").ToArray();
            Assert.AreEqual(1, matches.Length);

            matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "not (three or ten) one").ToArray();
            Assert.AreEqual(2, matches.Length);

            matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "not (three or ten) not(two or ten)").ToArray();
            Assert.AreEqual(1, matches.Length);

            matches = this.sampleInput.FilterUsingAntlr(d => d.Title, "not (four or ten) not \"two three\"").ToArray();
            Assert.AreEqual(2, matches.Length);
        }
    }
}
